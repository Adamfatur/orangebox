...
try:
    ...
    SevenServoHardware = None

try:
    from core.bin_monitor import BinMonitor
except Exception:
    BinMonitor = None
...
class MainController:
    ...
    def __init__(self, classifier, hardware_interface):
        ...
        # Auto-select servo hardware berdasarkan config
        driver = getattr(config, 'SERVO_DRIVER', 'seven_servo').lower()
        
        if driver == 'seven_servo' and SevenServoHardware is not None:
            print("[MainController] Using 7-Servo hardware with locking system")
            self.servo_hw = SevenServoHardware()
        else:
            print("[MainController] ⚠️  Warning: SERVO_DRIVER should be 'seven_servo'")
            print("[MainController] Falling back to 7-Servo system...")
            self.servo_hw = SevenServoHardware() if SevenServoHardware else None
        ...
        # Bin Capacity Monitor (Ultrasonic sensors + Blynk)
        self.bin_monitor = None
        self.enable_bin_monitoring = getattr(config, 'ENABLE_BIN_MONITORING', False)
        if self.enable_bin_monitoring and BinMonitor is not None:
            try:
                print("[MainController] → Initializing Bin Capacity Monitor...")
                self.bin_monitor = BinMonitor(
                    enable_blynk=True,
                    database_service=self.database_service if self.enable_database else None,
                    device_id=self.device_id,
                    location_service=self.location_service if self.enable_gps else None
                )
                self.bin_monitor.start_monitoring()
                print("[MainController] ✓ Bin monitoring initialized and started")
                if self.database_service:
                    print("[MainController]   → Database logging enabled (every 60s)")
                if self.location_service:
                    print("[MainController]   → GPS location tracking enabled")
            except Exception as e:
                print(f"[MainController] ⚠️  Bin monitor initialization failed: {e}")
                print("[MainController] ℹ️  Continuing without bin capacity monitoring...")
                self.bin_monitor = None
                self.enable_bin_monitoring = False
        else:
            if not self.enable_bin_monitoring:
                print("[MainController] ℹ️  Bin monitoring disabled (ENABLE_BIN_MONITORING=False)")
            elif BinMonitor is None:
                print("[MainController] ⚠️  BinMonitor class not available")
        ...
        if not skip_detection:
            ...
            if not self.in_cooldown and self.consecutive_motion >= self.motion_consecutive_required:
                ...
                if motion_detected:
                    ...
                    # Add bin capacity info if monitoring enabled
                    if self.bin_monitor and self.enable_bin_monitoring:
                        bin_status = self.bin_monitor.get_status_dict()
                        status_info['bin_a_level'] = bin_status['bin_a_level']
                        status_info['bin_b_level'] = bin_status['bin_b_level']
                        status_info['bin_a_full'] = bin_status['bin_a_full']
                        status_info['bin_b_full'] = bin_status['bin_b_full']
    ...
    def _state_classifying(self) -> State:
        ...
        # ═══════════════════════════════════════════════════════════
        if self.bin_monitor and self.enable_bin_monitoring:
            bin_name = 'A' if decision == State.SORTING_A else 'B'
            bin_level = self.bin_monitor.get_bin_level(bin_name)
            is_full = self.bin_monitor.is_bin_full(bin_name)
            
            print(f"\n[CLASSIFYING] 📊 BIN {bin_name} Capacity Check:")
            print(f"              Current level: {bin_level:.1f}%")
            print(f"              Status: {'🔴 FULL' if is_full else '🟢 OK'}")
            
            if is_full:
                # Bin penuh - abort sorting
                print(f"\n[CLASSIFYING] ⚠️  WARNING: BIN {bin_name} IS FULL ({bin_level:.1f}%)!")
                print(f"[CLASSIFYING] ❌ SORTING ABORTED - Please empty the bin first")
                
                # Tampilkan warning ke display
                warning_info = {
                    'label': label,
                    'confidence': confidence,
                    'bin_full': True,
                ...
                for _ in range(25):  # 5 seconds at 20ms per frame
                    self.hw.display_frame(
                        self.last_full_frame,
                        f"⚠️  BIN {bin_name} FULL ({bin_level:.0f}%) - CANNOT SORT",
                        warning_info
                    )
                ...
                # Update stats for rejected items
                self.stats['low_confidence_count'] += 1  # Reuse this counter for "bin full" rejections
                
                # Release processing lock and return to IDLE WITHOUT cooldown
                # (Allow immediate re-trigger when bin is emptied)
                self.is_processing = False
                return State.IDLE
            else:
                print(f"[CLASSIFYING] ✓ BIN {bin_name} has capacity - proceeding with sorting")
    ...
    def _shutdown(self):
        ...
        # Cleanup bin monitoring
        if self.bin_monitor:
            self.bin_monitor.cleanup()
