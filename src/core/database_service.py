...
class DatabaseService:
        ...
        try:
            ...
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    ...
                    # Table: bin_capacity_logs
                    # Menyimpan history level kapasitas tong sampah
                    cursor.execute("""
                        CREATE TABLE IF NOT EXISTS bin_capacity_logs (
                            ...
                            KEY idx_bin_status_composite (device_id, bin_a_status, bin_b_status, timestamp),
                            KEY idx_device_date (device_id, DATE(timestamp))
    ...
    def insert_bin_capacity_log(self, device_id: str,
                                bin_a_level: float, bin_a_distance: Optional[float],
                                bin_b_level: float, bin_b_distance: Optional[float],
                                latitude: Optional[float] = None,
                                longitude: Optional[float] = None,
                                warning_threshold: float = 70.0,
                                full_threshold: float = 85.0) -> bool:
        """
        Insert bin capacity log ke database.
        
        Args:
            device_id: Unique device identifier
            bin_a_level: Level BIN A dalam persen (0-100)
            bin_a_distance: Jarak sensor ke permukaan sampah BIN A (cm)
            bin_b_level: Level BIN B dalam persen (0-100)
            bin_b_distance: Jarak sensor ke permukaan sampah BIN B (cm)
            latitude: GPS latitude (optional)
            longitude: GPS longitude (optional)
            warning_threshold: Threshold untuk status WARNING (default: 70%)
            full_threshold: Threshold untuk status FULL (default: 85%)
        ...
        try:
            ...
            bin_a_status = get_status(bin_a_level)
            bin_b_status = get_status(bin_b_level)
            
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    sql = """
                        INSERT INTO bin_capacity_logs
                        (device_id, timestamp, 
                         bin_a_level, bin_a_distance, bin_a_status,
                         bin_b_level, bin_b_distance, bin_b_status,
                    ...
                    # Log if any bin is FULL or WARNING
                    if bin_a_status in ('FULL', 'WARNING') or bin_b_status in ('WARNING', 'FULL'):
                        print(f"[DatabaseService] ⚠️  Bin capacity alert logged: "
                              f"BIN A: {bin_a_level:.1f}% ({bin_a_status}), "
                              f"BIN B: {bin_b_level:.1f}% ({bin_b_status})")
                    
                    return True
                    
        except Exception as e:
            print(f"[DatabaseService] Error inserting bin capacity log: {e}")
            return False
    
    def get_bin_capacity_status(self, device_id: str) -> Optional[Dict]:
        """
        Get status kapasitas tong terkini untuk device tertentu.
        
        Args:
            device_id: Unique device identifier
            
        Returns:
            Dictionary dengan status terkini atau None jika tidak ada data
        """
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    sql = """
                        SELECT 
                            ...
                            location_longitude,
                            CASE 
                                WHEN bin_a_status = 'FULL' OR bin_b_status = 'FULL' THEN 'URGENT'
                                WHEN bin_a_status = 'WARNING' OR bin_b_status = 'WARNING' THEN 'WARNING'
                                ELSE 'OK'
                            END AS overall_status,
                            TIMESTAMPDIFF(MINUTE, timestamp, NOW()) AS minutes_since_update
            ...
            print(f"[DatabaseService] Error fetching bin capacity status: {e}")
            return None
    
    def get_bin_capacity_alerts(self) -> List[Dict]:
        """
        Get semua device yang tong-nya penuh atau warning.
        
        Returns:
            List of dictionaries dengan alert info
        """
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cursor:
                    sql = """
                        ...
                        WHERE bcl.bin_a_status IN ('WARNING', 'FULL')
                           OR bcl.bin_b_status IN ('WARNING', 'FULL')
                        ORDER BY 
                            FIELD(CASE 
                                WHEN bcl.bin_a_status = 'FULL' OR bcl.bin_b_status = 'FULL' THEN 'URGENT'
                                ELSE 'WARNING'
                            END, 'URGENT', 'WARNING'),
                            minutes_since_update ASC
                    """
                    cursor.execute(sql)
                    return cursor.fetchall()
                    
        except Exception as e:
            print(f"[DatabaseService] Error fetching bin capacity alerts: {e}")
            return []
